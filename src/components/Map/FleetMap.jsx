...
function getDeviceCode(d) {
    return d.deviceCode || d.device_code || d.deviceNo || d.deviceSn || d.sn || d.imei || d.code || "";
}

function getUpdatedAt(d) {
    return d.updated_at || d.updatedAt || d.batteryTime || d.bindTime || null;
}

function isLiveDevice(d) {
    return Boolean(
        getDeviceCode(d) ||
        d.battery != null ||
        d.locationText ||
...
function getAlarmLevel(d) {
    if (d.freshlianceAlarmLevel) return d.freshlianceAlarmLevel;
    return isLiveDevice(d) ? "OK" : "OFFLINE";
}

function tempToColor(temp) {
    if (temp == null) return "#16a34a";
...
function AlarmHistory({ alarms = [] }) {
    if (!alarms.length) {
        return <div style={{ color: "#9ca3af", fontSize: 11 }}>Son alarm kaydı yok</div>;
    }
            ...
            {alarms.slice(0, 4).map((a, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11 }}>
                    ...
                    <span style={{ color: "#9ca3af", whiteSpace: "nowrap" }}>
                        {a.time
                            ? new Date(a.time).toLocaleString("tr-TR", {
                                day: "2-digit",
                                month: "2-digit",
                                hour: "2-digit",
                                minute: "2-digit",
...
function clusterDevices(devices, zoomLevel) {
    if (zoomLevel >= 10 || devices.length <= 1) {
        return devices.map((d) => ({ ...d, _cluster: false }));
    }
    ...
    devices.forEach((d, i) => {
        ...
        if (group.length === 1) {
            ...
            const hasAlarm = group.some((x) => getAlarmLevel(x) === "ALARM");
            ...
            result.push({
                ...
                freshlianceAlarmLevel: hasAlarm ? "ALARM" : getAlarmLevel(group[0]),
                _members: group,
            });
        }
    });
...
function MapInfoBar({ devices, onRefresh, refreshing, lastRefresh }) {
    ...
    const alarms = devices.filter((d) => getAlarmLevel(d) === "ALARM").length;
    const offline = devices.filter((d) => getAlarmLevel(d) === "OFFLINE").length;
    ...
    return (
        ...
        >
            <span style={{ fontWeight: 700, color: "#111827" }}>{total} Cihaz</span>
...
function TempLegend() {
    ...
    return (
        ...
        >
            <div style={{ fontWeight: 700, color: "#374151", marginBottom: 6 }}>Sıcaklık</div>
...
export default function FleetMap({ devices = [], onRefresh, autoRefresh = true }) {
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", height: "100%", position: "relative" }}>
            ...
            <MapInfoBar devices={devices} onRefresh={doRefresh} refreshing={refreshing} lastRefresh={lastRefresh} />
            ...
            <div style={{ flex: 1, position: "relative" }}>
                ...
                >
                    ...
                    {clustered.map((d, idx) => {
                        ...
                        const level = getAlarmLevel(d);
                        const cfg = STATUS_CONFIG[level] || STATUS_CONFIG.OFFLINE;
                        const temp = d.probe_raw?.temperature ?? d.temperature;
                        const markerColor = level === "OFFLINE" ? cfg.color : tempToColor(temp);
                        const deviceCode = getDeviceCode(d);
                        const updatedAt = getUpdatedAt(d);

                        const icon = d._cluster
                            ? createClusterIcon(d._clusterCount, cfg.color)
                            : createMarkerIcon(markerColor, level === "ALARM", deviceCode?.slice(-4));

                        return (
                            <Marker key={`${deviceCode || "device"}-${idx}`} position={[lat, lng]} icon={icon}>
                                    ...
                                    <div style={{ fontFamily: "system-ui, sans-serif", fontSize: 13 }}>
                                        ...
                                        >
                                            ...
                                            <strong style={{ fontSize: 14, color: "#111827" }}>
                                                {d._cluster ? `${d._clusterCount} Cihaz` : deviceCode || "Cihaz"}
                                            </strong>
                                        ...
                                        {d._cluster ? (
                                            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                                                {d._members.map((m, mi) => {
                                                    const ml = getAlarmLevel(m);
                                                    const mc = STATUS_CONFIG[ml] || STATUS_CONFIG.OFFLINE;
                                                    const mt = m.probe_raw?.temperature ?? m.temperature;
                                                    const mCode = getDeviceCode(m);
                                            ...
                                            <>
                                                <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 10 }}>
                                                    <tbody>
                                                        {[
                                                            ...
                                                            ["📍", "Konum", d.locationText || `${lat.toFixed(5)}, ${lng.toFixed(5)}`],
                                                            [
                                                                "🕒",
                                                                "Güncelleme",
                                                                updatedAt
                                                                    ? new Date(Number(updatedAt)).toLocaleString("tr-TR")
                                                                    : "-",
                                                            ],
                                                        ].map(([icon, label, value]) => (
                                                            <tr key={label}>
                                                                <td style={{ padding: "3px 0", color: "#9ca3af", width: 20 }}>
                                                                    {icon}
                                                                </td>
                                                ...
                                                {d.routeHistory?.length > 1 && (
                                                    <button
                                                        onClick={() =>
                                                            setSelectedRoute(
                                                                selectedRoute?.deviceCode === deviceCode
                                                                    ? null
                                                                    : { deviceCode, points: d.routeHistory }
                                                            )
                                                        }
                                                        style={{
                                                            marginTop: 10,
                                                            ...
                                                            borderRadius: 7,
                                                            background:
                                                                selectedRoute?.deviceCode === deviceCode ? "#eef2ff" : "#fff",
                                                            color: "#6366f1",
                                                            fontSize: 12,
                                                            fontWeight: 700,
                                                            cursor: "pointer",
                                                    ...
                                                    >
                                                        {selectedRoute?.deviceCode === deviceCode ? "✕ Rotayı Kaldır" : "🗺 Rotayı Göster"}
                                                    </button>
                                                )}
                                            </>
                                        )}
